...
export default async (req: Request, _context: Context): Promise<Response> => {
  ...
  const responderUrlParam =
    extractResponderUrlParam(req.url) || req.headers.get('x-ocsp-responder-url')
  ...
  try {
    ...
    return new Response(response.body, {
      ...
      headers: {
        ...CORS_HEADERS,
        'Content-Type': response.contentType || 'application/ocsp-response',
      },
    })
  } catch (err: unknown) {
