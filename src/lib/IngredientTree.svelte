<script lang="ts">
  ...
  function relationshipClass(rel?: string): string {
    switch (rel) {
      ...
      case 'componentOf': return 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
    ...
    <div class="flex items-start gap-4 p-4 rounded-2xl border transition-colors
      ...
      <div class="flex-shrink-0 w-10 h-10 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
        ...
        {:else if node.format?.startsWith('image/')}
          <svg class="w-5 h-5 text-gray-500 dark:text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
