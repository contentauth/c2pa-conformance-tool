              ...
              {#if ocspStatus !== 'unknown'}
                <div class="bg-gray-50 dark:bg-gray-900 rounded-xl p-4">
                  ...
                  {:else if ocspStatus === 'inaccessible'}
                    {#if report.ocspServerResult}
                      {#if report.ocspServerResult.status === 'good'}
                        <div class="flex items-center gap-2">
                          <svg class="w-4 h-4 text-green-500 dark:text-green-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                          <span class="text-sm font-medium text-green-700 dark:text-green-300">Not revoked <span class="font-normal text-gray-500 dark:text-gray-400">(server-side check)</span></span>
                        </div>
                      {:else if report.ocspServerResult.status === 'revoked'}
                        <div class="flex items-center gap-2">
                          ...
                          <span class="text-sm font-medium text-red-700 dark:text-red-300">Certificate revoked <span class="font-normal text-gray-500 dark:text-gray-400">(server-side check)</span></span>
                        </div>
                      {:else}
                        <div class="flex items-center gap-2">
                          <svg class="w-4 h-4 text-amber-500 dark:text-amber-300 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                          <span class="text-sm font-medium text-amber-700 dark:text-amber-300">OCSP server inaccessible from browser</span>
                        </div>
                      {/if}
                    {:else}
                      <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-amber-500 dark:text-amber-300 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                        <span class="text-sm font-medium text-amber-700 dark:text-amber-300">OCSP server inaccessible — checking via server…</span>
                      </div>
                    {/if}
                  {/if}
                </div>
