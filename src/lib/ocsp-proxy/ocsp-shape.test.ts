...
describe('extractOcspRequestFromGetPath', () => {
  ...
  it('extracts a base64-encoded OCSP request containing unencoded "/" characters (c2pa-rs GET path)', () => {
    // Exact base64 DER OCSPRequest generated by c2pa-rs for frogs5.signed.jpg (contains "/36B/lmEdxi")
    const b64WithSlashes =
      'MFUwUzBRME8wTTAJBgUrDgMCGgUABBSkCEor5Uq9Uyr0FLtRl2VZ377x5AQUU221cughPmOhi1sVfGH9198SMZsCFACTaFWeq0VQ61qRF/36B/lmEdxi'
    const url = new URL(`http://c2pa-ocsp.pki.goog/${b64WithSlashes}`)
    const extracted = extractOcspRequestFromGetPath(url)
    expect(extracted).not.toBeNull()
    expect(isValidOcspRequest(extracted!)).toBe(true)

    const prefixedUrl = new URL(`http://c2pa-ocsp.pki.goog/ocsp/v1/${b64WithSlashes}`)
    const extractedFromPrefixed = extractOcspRequestFromGetPath(prefixedUrl)
    expect(extractedFromPrefixed).not.toBeNull()
    expect(isValidOcspRequest(extractedFromPrefixed!)).toBe(true)
  })
